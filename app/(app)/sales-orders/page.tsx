...
export default function SalesOrdersListPage() {
  ...
  const remove = async (id: string) => {
    const ok = window.confirm("Are you sure you want to delete this sales order?");
    if (!ok) return;
    const res = await apiFetch(`/api/sales-orders?id=${id}`, { method: "DELETE" });
    const data = await res.json();
    if (!res.ok) {
  ...
  const approve = async (id: string, action: "approve" | "reject") => {
    const res = await apiFetch("/api/sales-orders/approve", {
      method: "POST",
    ...
    <div className="grid" style={{ gap: 24 }}>
      <div className="panel">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1>Sales Orders</h1>
            <p className="muted">Manage sales orders.</p>
          </div>
          <Link className="button" href="/sales-orders/new">Add Sales Order</Link>
        </div>
      </div>
      <div className="panel">
